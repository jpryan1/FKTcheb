...
function get_leg_table(p_size::Int)
    pij = zeros(p_size, p_size)
    pij[1,1] = 1
    pij[2,2] = 1
    for i in 3:p_size
        n=i-2
        for j in 1:i
            if j == 1
                pij[i,j] = (-n*pij[i-2,j])/(n+1)
            else
                pij[i,j] = ((2n+1)*pij[i-1,j-1] - n*pij[i-2,j])/(n+1)
...
function get_shifted_leg_table(p_size::Int)
    pij = zeros(p_size, p_size)
    pij[1,1] = 1
    pij[2,2] = 2*sqrt(3)
    pij[2,1] = -1*sqrt(3)
    ...
    for i in 3:p_size
        ...
        for j in 1:i
            if j == 1
                ...
                # pij[i,j] = (-(2n+1)*pij[i-1,j]
                #                 - n*pij[i-2,j]) /(n+1)

                pij[i,j] = (sqrt(2(i-1)+1)*
                    (-(2n+1)*(pij[i-1,j]/sqrt(2(i-2)+1))
                    - n*(pij[i-2,j]/sqrt(2(i-3)+1))) /(n+1))
            else
                # pij[i,j] = ((2n+1)*pij[i-1,j-1] - n*pij[i-2,j])/(n+1)
                #
                pij[i,j] = (sqrt(2(i-1)+1)*
                    (2(2n+1)*(pij[i-1,j-1]/sqrt(2(i-2)+1))
                    - (2n+1)*(pij[i-1,j]/sqrt(2(i-2)+1))
                    - n*(pij[i-2,j]/sqrt(2(i-3)+1)))/(n+1))


                # pij[i,j] = (2(2n+1)*pij[i-1,j-1]
                #     - (2n+1)*pij[i-1,j]
                #     - n*pij[i-2,j])/(n+1)
            end
        end
